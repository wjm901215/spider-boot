...
public class BeanUtils extends org.springframework.beans.BeanUtils {
	...
	public static void copyPropertiesToMap(Object bean, Map<String, Object> map) throws Exception {
		copyPropertiesToMap(bean, map, "");
	}
	 ...
	 * @param bean
	 * @param map
	 * @param prefix key的前缀，如prefix="acc"，属性名为name，则key为acc.name
	 * @throws Exception
	 */
	public static void copyPropertiesToMap(Object bean, Map<String, Object> map, String prefix) throws Exception {
		if(bean == null) return;
		...
		for(Method m : bean.getClass().getDeclaredMethods()) {
			...
			if(mn.startsWith("get") && mn.length() > 3) {
				String property = mn.replaceFirst("get", "");
				property = property.substring(0, 1).toLowerCase() + property.substring(1);
				map.put(prefix + property, m.invoke(bean));
			}
		}
	}
					...
					try {
						...
						if (value != null || readMethod.getReturnType().getName().equals("java.lang.String")) {// 这里判断以下value是否为空，当然这里也能进行一些特殊要求的处理 例如绑定时格式转换等等，如果是String类型，则不需要验证是否为空
							...
							if (!isEmpty) {
								...
								if(paramClass.getName().equals("java.sql.Timestamp")){

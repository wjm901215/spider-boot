package com.spider.ma.common.page;
...
public class Pager {
	...
	public String getOrderCondition() {
		...
		if (this.orderField != null && this.orderField.length() != 0) {
			//骆驼式命名转换为下滑线命名，数据库设计是以"_"命名规则，否则直接用orderField
			String orderFieldStr = CamelUtil.camelToUnderline(orderField);//orderField
			condition = " order by " + orderFieldStr
					+ (orderDirection ? " " : " desc ");
		}
		return condition;
	}

	public String getOracleQueryConditionStart() {
		String condition = "";
		condition = " SELECT * FROM ( SELECT A.*, ROWNUM RN FROM ( ";
		return condition;
	}
	public String getOracleQueryConditionEnd() {
		String condition = "";
		condition = " ) A WHERE ROWNUM  <="+ pageTail +") where RN >"+pageOffset;
		return condition;
	}
